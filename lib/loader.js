...
let _ = require('lodash'),
  loader = require('../../eddie-fs').loader,
  q = require('q'),
  io = require('../../eddie-io');
...
class FileLoader {
  ...
  getPattern(uses) {
    uses = this.explode(uses);
    let path = process.cwd() + '/.ed/' + 'libraries/' + uses.library + '/ed-config.json';
    return loader.loadFile(path).then((library)=> {
      ...
      if (pattern.library) {
        ...
        path = process.cwd() + '/.ed/' + 'libraries/' + uses.library + '/' + uses.version + '/ed-config.json'
        return loader.loadFile(path)
          .then((version)=> {
            pattern.version = version.content;
            pattern.version.scope = version.type;

            path = process.cwd() + '/.ed/' + 'libraries/' + uses.library + '/' + uses.version + '/' + uses.pattern + '.json';
