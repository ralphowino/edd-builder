...
class FileLoader {
  ...
  getPattern(uses) {
    uses = this.explode(uses);
    let path = 'libraries/' + uses.library + '/ed-config.json';
    return loader.loadFile(path).then((library)=> {
      ...
      if (pattern.library) {
        ...
        path = 'libraries/' + uses.library + '/' + uses.version + '/ed-config.json';
        return loader.loadFile(path)
          .then((version)=> {
            pattern.version = version.content;
            pattern.version.scope = version.type;

            path = 'libraries/' + uses.library + '/' + uses.version + '/' + uses.pattern + '.yaml';
