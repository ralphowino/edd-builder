      ...
      if (response.pattern.definitions) {
        _.forEach(response.pattern.definitions, function (value, attr) {
          ...
          if (_.isString(value)) {
            ...
            statement = '{% if not ' + attr + ' %}{% set ' + attr + ' %}' + value + '{% endset %}{% endif %}';
            if (attr == 'imports') {
              statement.concat('{% if not importx %}{% set importx = [{"import_name":"Illuminate\\Contracts\\Queue\\SelfHandling"},{"import_name":"Illuminate\\Contracts\\Queue\\ShouldQueue"},{"import_name":"Illuminate\\Queue\\SerializesModel"}]%}' +
                '{% endif %}');
            }
          }
